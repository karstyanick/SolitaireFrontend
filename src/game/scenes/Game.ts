export class SolitaireScene extends Phaser.Scene {
  ...
  create() {
    ...
    const cardElements: Phaser.GameObjects.Image[] = [];
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < cardsPerRow; col++) {
        ...
        const card = this.add
          .image(x, y, randomCard)
          .setInteractive({ draggable: true });

        card.name = randomCard;
        ...
        card.on("drop", (pointer: any, target: any) => {
          ...
          if (
            parseInt(targetCard.name.match(/\d+/)?.[0] || "") !==
            parseInt(card.name.match(/\d+/)?.[0] || "") + 1
          ) {
            console.log(
              `TargetCard Name: ${JSON.stringify(targetCard.toJSON())}`
            );
            console.log(`Card Name: ${JSON.stringify(card.toJSON())}`);
            card.x = card.input?.dragStartX || 0;
            card.y = card.input?.dragStartY || 0;
            return;
          }
